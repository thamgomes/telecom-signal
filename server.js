...
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'telecom-news.html')));
...
const YAHOO_STOCKS = [
  ...
  { sym:'AMXL34.SA', name:'Claro (América Móvil)', display:'AMXL34', color:'#CC1E1E' },
...
app.get('/api/news-febraban-rss', async (req, res) => {
  ...
  if (isFresh(cached)) return res.json(cached.data);
  try {
    const feed = await rssParser.parseURL(FEBRABAN_RSS_URL);
    ...
    const data = { items, fetchedAt: new Date().toISOString() };
    setCache('news-febraban-rss', data, 30 * 60 * 1000);
    res.json(data);
  } catch(e) {
    console.warn('[FebrabancRSS] ' + e.message);
    res.json({ items: [] });
  }
});
